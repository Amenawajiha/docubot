...
# Suppress noisy watchdog inotify debug messages which look like:
# DEBUG:watchdog.observers.inotify_buffer:in-event
watchdog_inotify_logger = logging.getLogger("watchdog.observers.inotify_buffer")
watchdog_inotify_logger.setLevel(logging.WARN)
...
def create_log_file(service_name):
    """Create log file (only once per session)"""

    # Check if file handler already exists
    if any(isinstance(h, logging.FileHandler) for h in logger.handlers):
        return  # Log file already created for this session

    log_dir = "logs"  # This folder will be in the path from where you run code
    ...
    fh = logging.FileHandler(log_location, mode="a", encoding="utf-8")
