"""FastAPI routes for the chatbot application."""
...
async def websocket_endpoint(fe_websocket: WebSocket):
    ...
    Args:
        ...
        user_id: User identifier (default: generates unique session ID)
    """
    ...
    try:
        while True:
            ...
            if validated_request.type == "store_message":
                try:
                    # Create server-validated message
                    # Role is FORCED to "user" and timestamp is SERVER-GENERATED
                    validated_message = ValidatedStoredMessage.from_client_payload(
                        payload=validated_request.message,
                        user_id=user_id
                    )
                    
                    # Convert to Message model for storage
                    message = Message(
        ...
        # On disconnect, persist session summary to DB for authenticated users
        try:
            if is_authenticated:
                summary_message = (
